...
WHERE u.id != $1
  AND EXISTS (
    ...
    WHERE user_id = $1 AND skill_id = us1.skill_id AND skill_type = 'learn'
  )
  AND EXISTS (
    SELECT 1 FROM user_skills 
    WHERE user_id = $1 AND skill_id = us2.skill_id AND skill_type = 'teach'
  )
LIMIT 10;
`
		...
		// ✅ FIX: initialize as an empty slice so JSON output is `[]`, not `null`
		matches := []map[string]interface{}{}

		for rows.Next() {
			var id int
			var name, bio, teachSkill, learnSkill string
			if err := rows.Scan(&id, &name, &bio, &teachSkill, &learnSkill); err != nil {
				http.Error(w, "Error scanning matches", http.StatusInternalServerError)
				return
			}
